...
public class SwipeManager : MonoBehaviour
{
    ...
    [SerializeField] float minSwipeLength = 0.3f;
    ...
    [SerializeField] bool useEightDirections = true;
 
    #endregion
 
    const float eightDirAngle = 0.8334f;//0.906f;
    ...
    static void DetectSwipe ()
    {
        ...
        if (GetTouchInput() || GetMouseInput()) {
            ...
            if (swipeEndTime - swipeStartTime > 0.5f && comeFromFunc)
            {
                swipeDirection = Swipe.LongClick;
                swipeEnded = true;
    ...
    static bool touchEnded = true;
    static bool comeFromFunc = false;
    static bool GetTouchInput ()
    {
        if (Input.touches.Length > 0) {
            ...
            // Still swiping/touching
                ...
                Vector2 currentSwipe = (Vector2)Input.mousePosition - firstPressPos;
                float swipeCm = currentSwipe.magnitude / dpcm;
                if (Time.time - swipeStartTime > 0.5f && !touchEnded && swipeCm < instance.minSwipeLength)
                {
                    comeFromFunc = true;
                    return true;
                }
